...
export const AppLayout: React.FC<AppLayoutProps> = ({ children }) => {
  ...
  const pageTitle = activePath.split("/").pop()?.replace("-", " ") || "Dashboard";
        ...
        <header className="sticky top-0 z-30 flex items-center justify-between px-6 py-4 bg-white/80 backdrop-blur-md border-b border-gray-200 shadow-sm">
          ...
          <div className="hidden lg:flex flex-col ml-2">
            ...
            <div className="flex items-center gap-2 text-xs text-gray-500 mt-0.5">
               ...
               <span>/</span>
               <span className="capitalize text-[#48A111] font-medium">{pageTitle}</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
              ...
              <div className="relative">
                ...
                >
                  <div className="p-1.5 bg-gray-100 rounded-lg group-hover:bg-[#48A111]/10 transition-colors">
                     <WarehouseIcon size={16} className="text-gray-500 group-hover:text-[#48A111]" />
                  </div>
                  <div className="text-left hidden sm:block">
                     <p className="text-[10px] text-gray-400 font-bold uppercase tracking-wider leading-none">Warehouse</p>
                  ...
                  <ChevronDown size={14} className={`text-gray-400 ml-1 transition-transform ${warehouseDropdownOpen ? 'rotate-180' : ''}`} />
                </button>
                    ...
                    <div className="absolute right-0 mt-2 w-64 bg-white rounded-xl shadow-xl border border-gray-100 z-20 overflow-hidden ring-1 ring-black/5 animate-in fade-in zoom-in-95 duration-200">
                      <div className="py-1 max-h-80 overflow-y-auto">
                        <div className="px-4 py-2 border-b border-gray-100">
                           <span className="text-xs font-semibold text-gray-500 uppercase">Select Location</span>
                        </div>
                        {(isOwner() || isManager()) && warehouses.length > 0 ? (
                          <>
                            ...
                            >
                              <span className={`${!selectedWarehouseId ? "text-[#48A111] font-bold" : "text-gray-700"}`}>All Warehouses</span>
                              {!selectedWarehouseId && <div className="w-2 h-2 rounded-full bg-[#48A111]"></div>}
                            </button>
                            {warehouses.map((w) => (
                              <button
                                key={w.id}
                              ...
                              >
                                <span className={`${selectedWarehouseId === w.id ? "text-[#48A111] font-bold" : "text-gray-700"}`}>{w.name}</span>
                                {selectedWarehouseId === w.id && <div className="w-2 h-2 rounded-full bg-[#48A111]"></div>}
                              </button>
                            ))}
                          </>
                        ) : (
            ...
            <button 
               onClick={() => navigate(isOwner() ? "/owner/alerts" : isManager() ? "/manager/alerts" : "/qc/alerts")}
            ...
            >
               <AlertTriangle size={20} className="text-gray-400 group-hover:text-red-500 transition-colors" />
               {alertCount > 0 && (
